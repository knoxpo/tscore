...
if command -v wrk >/dev/null 2>&1; then
    ...
    # while it was taken, and this row is the noisiest on the board. Three
    # sweeps cost ~2.5 minutes and buy the spread table underneath it.
    # Raise it if the spread looks wide; drop the flag for a single sweep.
    python3 "$DIR/http_bench.py" 8 --repeat 3 >>"$OUT" || echo "(http bench failed)" >>"$OUT"
else
